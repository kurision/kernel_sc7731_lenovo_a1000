...
static char gsensor_version[]="kionix_1.0";
static char gsensor_vendor_name[20]="kionix";
...
//	if(acceld->accel_drdy == 0)  //delete by hzb
		queue_delayed_work(acceld->accel_workqueue, &acceld->accel_work, 0);
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				if ((atomic_read(&acceld->accel_report_count) % (10*HZ/acceld->poll_delay)) == 0)
				{
					atomic_set(&acceld->accel_report_count,0); 
					printk(KERN_INFO "%s:Report--[0x%x,0x%x,0x%x]; delay is %d mS\n",__func__,
						acceld->accel_data[acceld->axis_map_x],acceld->accel_data[acceld->axis_map_y],
						acceld->accel_data[acceld->axis_map_z],jiffies_to_msecs(acceld->poll_delay));
				}
				atomic_inc(&acceld->accel_report_count);
...
static int kionix_accel_power_on(struct kionix_accel_driver *acceld)
{
	if (acceld->accel_pdata.power_on)
		return acceld->accel_pdata.power_on(1);
...
static void kionix_accel_work(struct work_struct *work)
{
	...
	mutex_lock(&acceld->mutex_data);
	
	if((atomic_read(&acceld->accel_enabled) == 0) ||(atomic_read(&acceld->accel_suspended) == 1))
	{
		mutex_unlock(&acceld->mutex_data);
		return;
	}
...
static void kionix_accel_update_direction(struct kionix_accel_driver *acceld)
{
	...
	//write_unlock(&acceld->rwlock_accel_data);
	printk(KERN_INFO "%s: %d;%d;%d;%d;%d;%d;%d;\n",__func__,direction,acceld->axis_map_x,acceld->axis_map_y,acceld->axis_map_z,acceld->negate_x,acceld->negate_y,acceld->negate_z);
	return;
}
...
//	mutex_lock(&acceld->mutex_resume);
	...
	if(atomic_read(&acceld->accel_enabled) > 0){
		...
		/*if(atomic_dec_and_test(&acceld->accel_enabled))*/ {
			...
			if(atomic_read(&acceld->accel_suspended) == 0) {
...
static void __init kionix_accel_init_input_device(struct kionix_accel_driver *acceld,
					      struct input_dev *input_dev)
{
...
static int __init kionix_accel_setup_input_device(struct kionix_accel_driver *acceld)
{
	struct input_dev *input_dev;
	int err;
						...
						const char *buf, size_t count)
{
	...
	if(kionix_strtok(buf, count, &buf2, enable_count) < 0) {
		...
		#endif
		printk(KERN_INFO "%s: enable = 0x%x\n",__func__,enable);
...
static ssize_t kionix_accel_vendor_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
	 return sprintf(buf, acceld->who_am_i);
}

static DEVICE_ATTR(vendor_name, S_IRUGO,
		kionix_accel_vendor_name_show, NULL);
...
static DEVICE_ATTR(position, S_IRUGO|S_IWUSR, kionix_accel_get_direct, kionix_accel_set_direct);
...
static int __init kionix_verify(struct kionix_accel_driver *acceld)
{
	...
	switch (retval) {
		case KIONIX_ACCEL_WHO_AM_I_KXTE9:
			acceld->who_am_i="this accelerometer is a KXTE9.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTF9:
			acceld->who_am_i="this accelerometer is a KXTF9.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTI9_1001:
			acceld->who_am_i="this accelerometer is a KXTI9-1001.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTIK_1004:
			acceld->who_am_i="this accelerometer is a KXTIK-1004.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTJ9_1005:
			acceld->who_am_i="this accelerometer is a KXTJ9-1005.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTJ9_1007:
			acceld->who_am_i="this accelerometer is a KXTJ9-1007.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXCJ9_1008:
			acceld->who_am_i="this accelerometer is a KXCJ9-1008.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTJ2_1009:
			acceld->who_am_i="this accelerometer is a KXTJ2-1009.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
			acceld->who_am_i="this accelerometer is a KXCJK-1013.\n";
			KMSGINF(&acceld->client->dev, acceld->who_am_i);
			break;
		default:
			break;
...
static int __init kionix_accel_probe(struct i2c_client *client,
				 const struct i2c_device_id *id)
{
	...
	if (accel_pdata->irq_gpio_cfg)
	{
		if (accel_pdata->irq_gpio_cfg(accel_pdata,1)<0)
		{
			printk(KERN_ERR "%s:Gpio config error!\n",__func__);
			return -EINVAL;
		}
	}
	client->irq=accel_pdata->irq;
	err = kionix_accel_power_on(acceld);
	if (err < 0)
		goto err_free_mem;
...
//+add by hzb
	gpio_request(client->irq, "kionix-irq"); // Replace with appropriate GPIO setup
...
err_free_mem:
	...
	if (accel_pdata->irq_gpio_cfg)
	{
		accel_pdata->irq_gpio_cfg(accel_pdata,0);
	}
	return err;
}

static int __exit kionix_accel_remove(struct i2c_client *client)
{
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
...
static struct i2c_driver kionix_accel_driver = {
	...
	.remove		= __exit_p(kionix_accel_remove),

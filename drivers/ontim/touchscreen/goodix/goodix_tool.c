...
static s32 goodix_tool_read(struct file *filp, const char __user *buff, unsigned long len, void *data);
...
//        || !strncmp(IC_TYPE, "GT813", 5))
    ...
    else
    {
        ...
        GTP_INFO("I2C function: without pre and end cmd!  IC_TYPE is %s\n",IC_TYPE);
    }
}
...
s32 init_wr_node(struct i2c_client *client)
{
    ...
    goodix_proc_entry = proc_create_data(GOODIX_ENTRY_NAME, 0666, NULL, &proc_fops, NULL);
...
Output:
	Return read length.
********************************************************/
static s32 goodix_tool_read(struct file *filp, const char __user *buff, unsigned long len, void *data)//( char *page, char **start, off_t off, int count, int *eof, void *data )
{
    GTP_DEBUG_FUNC();
    ...
    else if (!cmd_head.wr)
    {
	    ...
	    if(copy_to_user(&buff[loc], &cmd_head.data[GTP_ADDR_LENGTH], len))
	    {
    ...
    else if (8 == cmd_head.wr)  //Read driver version
    {
       ...
       s32 tmp_len;
	unsigned char page[40]={0};
       tmp_len = strlen(GTP_DRIVER_VERSION);
       memcpy(page, GTP_DRIVER_VERSION, tmp_len);
       page[tmp_len] = 0;
       if(copy_to_user(buff, page, tmp_len+1))
       {
           GTP_ERROR("copy_to_user failed.");
       }

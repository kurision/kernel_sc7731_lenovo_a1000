...
#include <linux/slab.h>//+modify by liuwei
#include "../sprdfb.h"
#include <linux/delay.h>
#include "../sprdfb_dev.h"
...
//+modify by liuwei
dsih_error_t mipi_dsih_wr_common_cmd(dsih_ctrl_t * instance, sprdfb_dev_dsi_cmds *cmds)
{
	int ret = 0;
	sprdfb_dev_dsi_cmds * dsi_cmd=NULL;
	uint8_t packet_type = 0;
	int tran_mode = 0;
	uint8_t* cmd;
	uint8_t* params = NULL;
	...
	if (!cmds)
		return 0;
	dsi_cmd = cmds;
	while(dsi_cmd->type != DSI_CMD_END)
	{
		if (dsi_cmd->p_cmd)
		{
		    cmd = dsi_cmd->p_cmd;
		...
		switch (dsi_cmd->type)
		{
			case DSI_CMD_DCS:
			{
				switch (dsi_cmd->len)
				{
					case 0:
					{
						printk(KERN_WARNING "%s: DSI DCS Cmd len = 0!!!\n",__func__);
						break;
				    	}
					case 1:
					{
					...
					default:
					{
						packet_type = 0x39; /* DCS long write/write_LUT command packet */
						tran_mode = DCS_LW_TX;
						param_length =dsi_cmd->len + 2;
						params = kzalloc(param_length, GFP_KERNEL);
						if (!params)	{
							return -ENOMEM;
						}
						params[0]=dsi_cmd->len & 0xFF;
						params[1]=dsi_cmd->len >>8;
						memcpy(params+2,cmd,dsi_cmd->len);
						break;
				    	}
				}
				//mipi_dsih_write_part(instance, R_DSI_HOST_CMD_MODE_CFG, 1, tran_mode, 1);
				ret = mipi_dsih_gen_wr_packet(instance, 0, packet_type, params, param_length);
				if (param_length != dsi_cmd->len) kfree(params);
				break;
		    	}
			case DSI_CMD_GEN:
			{
				switch (dsi_cmd->len)
				{
					case 0:
					{
						printk(KERN_WARNING "%s: DSI GEN Cmd len = 0!!!\n",__func__);
						break;
				    	}
					case 1:
					{
						packet_type = 0x13; /* Generic short write 1 param *///modify by liuwei
					...
					case 2:
					{
						packet_type = 0x23; /* Generic short write 0 param *///modify by liuwei
					...
					default:
					{
						packet_type = 0x29; /* Generic long write/write_LUT command packet *///modify by liuwei
						tran_mode = GEN_LW_TX;
						param_length=dsi_cmd->len + 2;
						params = kzalloc(param_length, GFP_KERNEL);
						if (!params)	{
				...
				//mipi_dsih_write_part(instance, R_DSI_HOST_CMD_MODE_CFG, 1, tran_mode, 1);
				ret = mipi_dsih_gen_wr_packet(instance, 0, packet_type, params, param_length);
				if (param_length == (dsi_cmd->len +2)) kfree(params);
				break;
		    	}
			case DSI_RD_CMD_DCS:
			{
				short len =dsi_cmd->len-1;
				packet_type = 0x37; /* Set Maximum Return Packet Size */
				param_length=sizeof(len);
				params = &len;
				mipi_dsih_gen_wr_packet(instance, 0, packet_type, params, param_length);
				packet_type = 0x6; /* DCS read */
				param_length=len;
				params = cmd +1;
				
				mipi_dsih_eotp_rx(instance, 0);
				mipi_dsih_eotp_tx(instance, 0);

				ret = mipi_dsih_gen_rd_packet(instance,  0,  packet_type,  0, *cmd,  len, params);

				mipi_dsih_eotp_rx(instance, 1);
				mipi_dsih_eotp_tx(instance, 1);
				if (ret == len) ret =0;
				break;
		    	}
			case DSI_CMD_DEALY:
			...
			case DSI_CMD_UDEALY://modify by liuwei
			{
				udelay(dsi_cmd->len);
				break;
		    	}
